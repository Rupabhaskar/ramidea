...
function AssignZonesForm({
  ...
  const [zoneIds, setZoneIds] = useState(() => [...client.zoneIds]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 min-w-0">
      ...
      <div className="flex gap-2 pt-2">
        <Button type="button" variant="outline" className="flex-1 min-w-0" onClick={onClose}>
          Cancel
        </Button>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        {open && client ? (
          <AssignZonesForm key={client.id} client={client} onClose={() => onOpenChange(false)} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
